# Helidon Quickstart Inject Example
...
```shell
mvn package
java -jar target/helidon-quickstart-inject.jar
```
...
```shell
docker build -t helidon-quickstart-inject .
```

## Start the application with Docker

```shell
docker run --rm -p 8080:8080 helidon-quickstart-inject:latest
```
...
```shell
./target/helidon-quickstart-inject-jri/bin/start
```
...
```shell
docker build -t helidon-quickstart-inject-jri -f Dockerfile.jlink .
```

Start the application:

```shell
docker run --rm -p 8080:8080 helidon-quickstart-inject-jri:latest
```

See the start script help:

```shell
docker run --rm helidon-quickstart-inject-jri:latest --help
```