...
class PokemonMapperSelectionTest {
    ...
    @Test
    void generatedRepositoryImplementsInheritedLookup() throws Exception {
        String repository = generatedSource("model/PokemonRepository__Jdbc.java");

        assertThat(repository, containsString("public Optional<Pokemon> findByName(String name)"));
        assertThat(repository, containsString("JdbcClient.Statement jdbcStatement = jdbcClient.create(SQL_FIND_BY_NAME)"));
        assertThat(repository, containsString("jdbcStatement.bind(1, name)"));
        assertThat(repository, containsString("return jdbcStatement.map(pokemonRowMapper).optional()"));
    }
